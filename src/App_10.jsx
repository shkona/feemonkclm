...
const GLOBAL_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  ══════════════════════════════════════════ */
  .fm-table-wrap { overflow-x: auto; border-radius: 0 0 16px 16px; }
...
function LeadDetail({leadId, currentUser, onBack, onRefresh}) {
  ...
  return (
    <div style={{maxWidth:"900px", margin:"0 auto"}}>
      ...
      {/* Progress bar */}
      {!["REJECTED","WENT_COLD"].includes(lead.status) && (
      ...
      {isMgmt && lead.status === "LEAD_CREATED" && (
        <div className={"fm-approval-banner " + (lead.created_by===currentUser.id ? "own-lead" : "pending")}>
          {showReject ? (
            <div style={{width:"100%"}}>
              <div style={{fontSize:"13px", fontWeight:700, color:"var(--text)", marginBottom:"8px"}}>Reason for rejection</div>
            ...
            <>
              <div>
                <div style={{fontSize:"13px", fontWeight:700, color: lead.created_by===currentUser.id ? "#1d4ed8" : "var(--yellow)"}}>
                  {lead.created_by===currentUser.id ? "You created this lead" : "Pending Your Vetting"}
                </div>
                <div style={{fontSize:"12px", color: lead.created_by===currentUser.id ? "#3b82f6" : "#d97706", marginTop:"2px"}}>
                  Approve to move to proposal stage.
                </div>
              </div>
              <div style={{display:"flex", gap:"8px"}}>
                {lead.created_by !== currentUser.id && <button onClick={() => setShowReject(true)} className="fm-btn fm-btn-danger fm-btn-sm">Reject</button>}
      ...
      {lead.status === "REJECTED" && (
        <div style={{marginTop:"16px", background:"var(--red-bg)", border:"1px solid var(--red-bd)", borderRadius:"8px", padding:"14px 16px", fontSize:"13px", color:"var(--red)"}}>
          <strong>Rejected:</strong> {lead.rejection_reason}
        </div>
      )}
...
const TERMINAL_STATUSES = new Set(["COMPLETED", "REJECTED", "WENT_COLD"]);
...
function Pipeline({currentUser, onSelectLead}) {
                    ...
                    <td style={{textAlign:"center"}}>
                      <ActionMenu
                        ...
                        onStatusChange={() => {
                          ...
                          setLeads(prev => prev.map(x => x.id === l.id ? {...x, _refreshing: true} : x));
                          loadData();
                        }}
                      />
                    </td>
...
export default function App() {
  ...
  if (!currentUser) return (<><InjectCSS /><LoginScreen onLogin={setCurrentUser} /></>);
      ...
      <div className="fm-layout">
        ...
        <nav className="fm-sidebar">
          ...
          {navItems.map(({v, label}) => (
            <button
              key={v}
              onClick={() => { setView(v); setSelectedLeadId(null); }}
              className={"fm-sb-item" + (view===v ? " active" : "")}>
              <span className="fm-sb-icon"><NavIcon name={v} /></span>
              {label}
            </button>
          ))}
        ...
        <div className="fm-main">
          <header className="fm-topbar">
            <div className="fm-topbar-title">
              {view === "pipeline" && selectedLeadId ? "Lead Detail" : PAGE_TITLES[view] || ""}
            </div>
          ...
          <main className="fm-content">
            ...
            {view === "pipeline" && !selectedLeadId && (
              <div>
                <div style={{marginBottom:"20px"}}>
                  <div style={{fontSize:"18px", fontWeight:700, color:"var(--text)"}}>Pipeline</div>
                  <div style={{fontSize:"13px", color:"var(--muted)", marginTop:"2px"}}>{isMgmt ? "All leads across the team." : "Your submitted leads."}</div>
                </div>
                <Pipeline currentUser={currentUser} onSelectLead={id => setSelectedLeadId(id)} />
              </div>
            )}
            {view === "pipeline" && selectedLeadId && (
